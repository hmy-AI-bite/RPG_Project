...
std::vector<Item*> Slime::GenerateDrops() const
{
    ...
    if (rand() % 100 < 30)
        drops.push_back(Potion::CreateSmallHealthPotion().release());
    return drops;
}
...
std::vector<Item*> Skeleton::GenerateDrops() const
{
    std::vector<Item*> drops;
    if (rand() % 100 < 25)
        drops.push_back(Potion::CreateMediumHealthPotion().release());
    if (rand() % 100 < 10)
        drops.push_back(Potion::CreateStrengthPotion().release());
    return drops;
}
...
{
    std::vector<Item*> drops;
    if (rand() % 100 < 30) drops.push_back(Potion::CreateAgilityPotion().release());
    if (rand() % 100 < 15) drops.push_back(Weapon::CreateStaff().release());
    return drops;
}
...
FireLizard::FireLizard(int level)
    : Enemy("火蜥蜴", 35 + level * 4, 7 + level * 3, 1 + level, level)
{
    agility = 15 + level * 2;
    speed = 90;
    spellDefense = 3;
    elementType = ElementType::Fire;
    experienceReward = 25 + level * 5;
    goldReward = 15 + level * 3;
    AddSkill(std::make_shared<MagicalSkill>("火焰吐息", 1.4, 10, 30 + level * 5, ElementType::Fire));
}
...
std::vector<std::shared_ptr<Enemy>> FireLizard::PerformAction(Character* target)
{
    ...
    {
        PerformPhysicalAttack(target, 1.0, " 用爪子抓！");
    }
    return {};
}

std::vector<Item*> FireLizard::GenerateDrops() const
{
    std::vector<Item*> drops;
    if (rand() % 100 < 25)
        drops.push_back(Potion::CreateSmallHealthPotion().release());
...
WindSprite::WindSprite(int level)
    : Enemy("风精灵", 28 + level * 3, 6 + level * 2, 1 + level, level)
{
    agility = 25 + level * 3;
    speed = 130;
    spellDefense = 4;
    ...
    goldReward = 12 + level * 2;
    AddSkill(std::make_shared<MagicalSkill>("风刃", 1.5, 12, 35 + level * 5, ElementType::Wind));
}
...
std::vector<std::shared_ptr<Enemy>> WindSprite::PerformAction(Character* target)
{
    ...
    {
        PerformPhysicalAttack(target, 1.0, " 用风刃切割！");
    }
    return {};
}

std::vector<Item*> WindSprite::GenerateDrops() const
{
    std::vector<Item*> drops;
    if (rand() % 100 < 30)
        drops.push_back(Potion::CreateSpeedPotion().release());
    return drops;
}
...
{
    std::vector<Item*> drops;
    drops.push_back(Weapon::CreateGreatSword().release());   // 必掉巨剑
    drops.push_back(Potion::CreateDefensePotion().release()); // 必掉护盾药水
    if (rand() % 100 < 50) drops.push_back(Weapon::CreateShield().release());
    return drops;
}
